...
This is especially important for ``ClusterFileStorage`` and ``CloudFileStorage``, which may have background threads and network connections that need to be properly closed.